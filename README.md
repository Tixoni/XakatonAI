...
│   ├── detector.py        # Детектор YOLO11
│   ├── processors.py      # Обработка изображений и видео
│   ├── filters.py         # Цветовые фильтры
│   ├── image_utils.py     # Утилиты для работы с изображениями
│   ├── tracker.py         # Трекер объектов (re-identification)
│   ├── reid.py            # Извлечение признаков для трекинга
│   └── ocr_reader.py      # OCR для распознавания номеров поездов
